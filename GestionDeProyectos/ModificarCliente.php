    ...
    <body>
        <?php
            ...
            try {
                ...
                # Se reciben desde el input 'Cliente' y 'Link'
                ...
                if (count($result) < 1) {
                    ...
                    # Si no está, se añade con el link del cliente cuyo nombre se quiere cambiar,
                    # Por otro lado, si está, se usa ese cliente
                    
                    # Nombre de cliente no está ocupado
                    if (count($result1) <= 0) {
                        # Se obtiene ID de cliente para proyecto de antes
                        $query2="SELECT ID_Cliente FROM Proyecto WHERE Link=:Link";
                        $stmt2 = $pdo->prepare($query2);
                        $stmt2->bindValue(':Link', $Link, PDO::PARAM_STR);
                        $stmt2->execute();
                        $result3 = $stmt2->fetchAll();
                        $id_cliente = $result3[0]['ID_Cliente'];
                    
                    
                        # Ver cuántos proyectos tienen este id
                        $query3="SELECT ID FROM Proyecto WHERE ID_Cliente=:id_cliente";
                        $stmt3 = $pdo->prepare($query3);
                        $stmt3->bindParam(':id_cliente', $id_cliente);
                        $stmt3->execute();
                        $result2 = $stmt3->fetchAll();
                    
                        # Si un proyecto o menos, entonces modificar el nombre, pues fue error de tipeo
                        if (count($result2) <= 1) {
                            # Eliminar de bd cliente
                            $query4="UPDATE Cliente 
                                    SET Nombre=:Cliente WHERE ID=:id_cliente";
                            $stmt4 = $pdo->prepare($query4);
                            $stmt4->bindValue(':Cliente', $Cliente, PDO::PARAM_STR);
                            $stmt4->bindParam(':id_cliente', $id_cliente);
                            $stmt4->execute();
                        
                        } else {
                            # Si es más de un proyecto, el antiguo nombre lo están ocupando
                            # más proyectos, así que se creará nuevo cliente para asignarle al proyecto
                        
                            # Se obtiene link del nombre del cliente que se quiere modificar
                            $query5 = "SELECT Link FROM Cliente WHERE ID=:id_cliente";
                            $stmt5 = $pdo->prepare($query5);
                            $stmt5->bindParam(':id_cliente', $id_cliente);
                            $stmt5->execute();
                            $result3 = $stmt5->fetchAll();
                            $link_cliente = $result3[0]['Link'];
                        
                            # Se insertan valores de link y nombre nuevo
                            $query6="INSERT INTO Cliente VALUES (:link_cliente, :Cliente)";
                            $stmt6 = $pdo->prepare($query6);
                            $stmt6->bindValue(':link_cliente', $link_cliente, PDO::PARAM_STR);
                            $stmt6->bindValue(':Cliente', $Cliente, PDO::PARAM_STR);
                            $stmt6->execute();
                            $id = $pdo->lastInsertId();
                        
                            $query7="UPDATE Proyecto 
                                    SET ID_Cliente=:id WHERE Link=:Link";
                        ...
                        $id_client = $result1[0]['ID'];
                        # Se obtiene ID de cliente para proyecto de antes
                        $query8="SELECT ID_Cliente FROM Proyecto WHERE Link=:Link";
                        $stmt8 = $pdo->prepare($query8);
                        $stmt8->bindValue(':Link', $Link, PDO::PARAM_STR);
                        $stmt8->execute();
                        $result4 = $stmt8->fetchAll();
                        $id_cliente = $result4[0]['ID_Cliente'];
                    
                        # Ver cuántos proyectos tienen este id
                        $query9="SELECT ID FROM Proyecto WHERE ID_Cliente=:id_cliente";
                        $stmt9 = $pdo->prepare($query9);
                        $stmt9->bindParam(':id_cliente', $id_cliente);
                        $stmt9->execute();
                        $result5 = $stmt9->fetchAll();
                    
                        # Si hay un proyecto, entonces se borrará el cliente correspondiente al antiguo nombre
                        if (count($result5) <= 1) {
                            # Se obtiene nombre de cliente para proyecto de antes
                            $query10="SELECT Nombre FROM Cliente WHERE ID=:id_cliente";
                            $stmt10 = $pdo->prepare($query10);
                            $stmt10->bindParam(':id_cliente', $id_cliente);
                            $stmt10->execute();
                            $result6 = $stmt10->fetchAll();
                            $nombre_cliente = $result6[0]['Nombre'];
                        
                            # Se realiza cambio de ID_Cliente en Proyecto correspondiente al link introducido
                            $query11="UPDATE Proyecto 
                                        SET ID_Cliente=:id_client 
                                        WHERE Link=:Link";
                            $stmt11 = $pdo->prepare($query11);
                            $stmt11->bindParam(':id_client', $id_client);
                            ...
                            if($nombre_cliente != $Cliente) {
                                ...
                                $query12="DELETE FROM Cliente 
                                            WHERE ID=:id_cliente";
                                $stmt12 = $pdo->prepare($query12);
                                $stmt12->bindParam(':id_cliente', $id_cliente);
                                $stmt12->execute();
                            }
                        } else {
                            ...
                            $stmt13->bindParam(':id_client', $id_client);

...
<body>
    <?php
        ...
        try {
            ...
            # Se reciben desde el input 'input1'
            $variable1=$_GET['input1'];
            $variable2=$_GET['input1'];

            # Consulta para mostrar proyectos en base a una palabra clave
            $query="SELECT Proyecto.ID, Cliente.Nombre as Nombre_Cliente, Cliente.Link as Link_Cliente, Proyecto.Nombre_Proyecto, Proyecto.Categoría, Proyecto.Fecha,
                        ...
                        FROM Proyecto
                            ...
                            WHERE Proyecto.Entregables LIKE :valor1 
                                    OR Proyecto.Objetivo_del_Proyecto LIKE :valor2 
                                ORDER BY Cliente.Nombre ASC";
            $stmt = $pdo->prepare($query);
            $stmt->bindValue(':valor1', "%$variable1%", PDO::PARAM_STR);
            $stmt->bindValue(':valor2', "%$variable2%", PDO::PARAM_STR);
            $stmt->execute();
            ...
            echo "</table>";
            echo "<a href='index.php'><div style='background-color: white; color: black; padding: 10px 0px 10px 0px; width:200px; margin-left:650px; text-align:center;border-radius:20px; margin-top:50px; border:2px solid black;'>Volver a buscador de proyectos.</div></a>";
        }
